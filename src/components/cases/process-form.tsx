    ...
    <div className="flex flex-col h-full bg-[#0a0f1e]">
      ...
      <ScrollArea className="flex-1">
        <div className="p-10 max-w-4xl mx-auto">
          <div className="mb-4">
            <span className="text-[10px] font-black text-primary uppercase tracking-[0.3em]">Etapa {currentStep} de 6</span>
          </div>
          ...
          {currentStep === 1 && (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2">
              <StepHeader 
                ...
                subtitle="Identifique o cliente principal e, se aplicável, co-autores do processo"
                icon={Users}
              />
              <div className="p-8 rounded-2xl border border-white/5 bg-white/[0.02] space-y-8">
                <div className="space-y-3">
                  ...
                  <Select value={formData.poloProcessual} onValueChange={(v) => handleInputChange("poloProcessual", v)}>
                    <SelectTrigger className="bg-black/20 border-white/10 h-14 text-white">
                      <SelectValue />
                    </SelectTrigger>
          ...
          {currentStep === 2 && (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2">
              <StepHeader 
                title="Polo Passivo" 
                subtitle="Identifique a parte contrária, empresas ou pessoas reclamadas"
                icon={Building2}
              />
              <div className="p-8 rounded-2xl border border-white/5 bg-white/[0.02] space-y-8">
                <div className="space-y-3">
                  ...
                  <Input 
                    ...
                    onChange={(e) => handleInputChange("defendantName", e.target.value.toUpperCase())}
                    className="bg-black/20 border-white/10 h-14 text-white"
                    placeholder="EX: EMPRESA DE LOGÍSTICA S.A."
                  />
                </div>
                <div className="space-y-3">
                  ...
                  <Input 
                    ...
                    onChange={(e) => handleInputChange("defendantDocument", e.target.value)}
                    className="bg-black/20 border-white/10 h-14 text-white"
          ...
          {currentStep === 3 && (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2">
              <StepHeader 
                ...
                subtitle="Informações numéricas e cronológicas do dossiê"
                icon={FileText}
              />
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 p-8 rounded-2xl border border-white/5 bg-white/[0.02]">
                <div className="space-y-3">
                  ...
                  <Input 
                    ...
                    className="bg-black/20 border-white/10 h-14 text-white font-mono"
                ...
                <div className="space-y-3">
                  <Label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">VALOR DA CAUSA (ESTIMADO)</Label>
                  <Input 
                ...
                <div className="space-y-3">
                  <Label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">DATA DE DISTRIBUIÇÃO</Label>
                  <Input 
                    type="date"
          ...
          {currentStep === 4 && (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2">
              <StepHeader 
                title="Jurisdição" 
                subtitle="Localização física e autoridade judiciária"
                icon={Gavel}
              />
              <div className="space-y-8 p-8 rounded-2xl border border-white/5 bg-white/[0.02]">
                ...
                <div className="grid grid-cols-2 gap-8">
                  <div className="space-y-3">
                    ...
                    <Input 
                      value={formData.vara} 
                      onChange={(e) => handleInputChange("vara", e.target.value.toUpperCase())}
                      className="bg-black/20 border-white/10 h-14 text-white"
                      placeholder="EX: 45ª VARA DO TRABALHO"
                    />
                  </div>
                  <div className="space-y-3">
                    <Label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">COMARCA / CIDADE</Label>
                    <Input 
                      value={formData.city} 
          ...
          {currentStep === 5 && (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2">
              <StepHeader 
                title="Corpo Técnico" 
                subtitle="Defina o advogado de frente e a equipe de apoio"
                icon={UserCheck}
              />
              <div className="p-8 rounded-2xl border border-white/5 bg-white/[0.02] space-y-8">
                <div className="space-y-3">
                  ...
                  <Select value={formData.responsibleStaffId} onValueChange={(v) => handleInputChange("responsibleStaffId", v)}>
                    ...
                    <SelectContent className="bg-[#0d121f] border-white/10 text-white">
                      <SelectItem value={user?.uid || "current"}>{user?.displayName || "Mim mesmo"}</SelectItem>
                      <SelectItem value="apoio">Equipe de Apoio RGMJ</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                <div className="flex items-center gap-4 p-6 rounded-xl bg-primary/5 border border-primary/20">
                  ...
                  <p className="text-xs text-muted-foreground leading-relaxed">
                    O responsável terá controle total sobre os prazos e publicações vinculadas a este número de processo.
                  </p>
                </div>
              </div>
            </div>
          ...
          {currentStep === 6 && (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2">
              <StepHeader 
                ...
                subtitle="Definição de teses jurídicas e observações táticas"
                icon={Target}
              />
              <div className="space-y-8 p-8 rounded-2xl border border-white/5 bg-white/[0.02]">
                <div className="space-y-3">
                  <Label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">RESUMO DO OBJETO (DESCRIÇÃO)</Label>
                  <Input 
                    value={formData.description} 
                    onChange={(e) => handleInputChange("description", e.target.value.toUpperCase())}
                    className="bg-black/20 border-white/10 h-14 text-white"
                    placeholder="EX: AÇÃO TRABALHISTA - HORAS EXTRAS E EQUIPARAÇÃO"
                  />
                </div>
                <div className="space-y-3">
                  <Label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">NOTAS ESTRATÉGICAS</Label>
                  <textarea 
                    value={formData.strategyNotes}
                    onChange={(e) => handleInputChange("strategyNotes", e.target.value)}
                    className="w-full bg-black/20 border-white/10 border rounded-md min-h-[150px] p-4 text-white text-sm focus:ring-1 focus:ring-primary/50 outline-none resize-none"
                    placeholder="Descreva as teses principais, provas essenciais e riscos identificados..."
                  />
                </div>
              </div>
            </div>
      ...
      <div className="p-8 bg-[#0a0f1e] border-t border-white/5 flex items-center justify-between">
        <Button 
          ...
          className="glass border-white/10 text-muted-foreground h-14 px-8 gap-3 uppercase font-black text-[11px] tracking-widest"
        >
        ...
        {currentStep < 6 ? (
          <Button 
            onClick={handleNext}
            className="bg-[#f5d030] hover:bg-[#d4af37] text-[#0a0f1e] h-14 px-10 gap-3 uppercase font-black text-[11px] tracking-widest shadow-xl transition-all"
          >
          ...
          <Button 
            ...
            className="bg-[#f5d030] hover:bg-[#d4af37] text-[#0a0f1e] h-14 px-12 gap-3 uppercase font-black text-[11px] tracking-widest shadow-xl transition-all"
          >
