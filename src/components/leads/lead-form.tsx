...
export function LeadForm({ 
  ...
  return (
    <div className="flex flex-col h-full bg-[#0a0f1e] font-sans overflow-hidden">
      <div className="px-6 pt-6 pb-2 flex-none bg-[#0a0f1e]/50 border-b border-white/5">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="bg-[#1a1f2e] w-full p-1 h-12 rounded-xl gap-1">
            <TabsTrigger value="autor" className="flex-1 text-[9px] uppercase font-black data-[state=active]:bg-primary data-[state=active]:text-background rounded-lg gap-2 h-full transition-all">
              <User className="h-3.5 w-3.5" /> AUTOR
            </TabsTrigger>
            <TabsTrigger value="reu" className="flex-1 text-[9px] uppercase font-black data-[state=active]:bg-primary data-[state=active]:text-background rounded-lg gap-2 h-full transition-all">
              <Building className="h-3.5 w-3.5" /> RÉU
            </TabsTrigger>
            <TabsTrigger value="demanda" className="flex-1 text-[9px] uppercase font-black data-[state=active]:bg-primary data-[state=active]:text-background rounded-lg gap-2 h-full transition-all">
      ...
      <ScrollArea className="flex-1 bg-[#0a0f1e]/20">
        <div className="px-6 py-6 max-w-4xl mx-auto space-y-10 pb-32">
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-2" ref={searchRef}>
                  ...
                  <div className="relative" onClick={() => !lockMode && setIsSearchOpen(true)}>
                    <Input placeholder="PESQUISAR OU INSERIR NOME..." className={cn(inputClass, "h-12 text-sm", lockMode && "opacity-60")} value={formData.name || searchTerm} readOnly={!isSearchOpen && !lockMode} />
                  </div>
                </div>
          ...
          {activeTab === "demanda" && (
            <div className="space-y-8 animate-in fade-in duration-500">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2" ref={courtSearchRef}>
              ...
              <div className="space-y-4">
                ...
                <Textarea className="bg-black/40 border-white/5 min-h-[150px] text-white text-xs uppercase resize-none p-4 rounded-xl" value={formData.notes} onChange={(e) => handleInputChange("notes", e.target.value.toUpperCase())} />
              </div>
            </div>
          )}
      ...
      <div className="p-6 bg-[#0a0f1e] border-t border-white/5 flex items-center justify-between shadow-2xl z-30 flex-none">
        <Button variant="ghost" className="text-muted-foreground font-black uppercase text-[9px] tracking-widest px-6 h-10" onClick={onCancel}>ABORTAR</Button>
        <Button onClick={handleSubmit} className="gold-gradient text-background font-black h-12 text-[10px] uppercase tracking-widest shadow-xl rounded-lg px-10 flex items-center gap-2">
          <ShieldCheck className="h-4 w-4" /> SALVAR DOSSIÊ
        </Button>
      </div>
    </div>
  )
