  ...
  if (loadingLeads || loadingCases || loadingHearings || !canQuery) {
    return (
      <div className="h-[60vh] flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-white/40">Sincronizando Centro de Comando...</p>
      </div>
    )
  }
    ...
    <div className="space-y-10 animate-in fade-in duration-700">
      <div className="space-y-1">
        <h1 className="text-4xl font-bold text-white tracking-tight">
          Bem-vindo, <span className="text-white">{displayName}</span>
        </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, i) => (
          <Card key={i} className="glass border-white/5 shadow-2xl hover:border-primary/20 transition-all group">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <Card className="lg:col-span-2 glass border-white/5 shadow-2xl min-h-[400px]">
          <CardHeader className="flex flex-row items-center justify-between border-b border-white/5 mb-6">
            <CardTitle className="text-xs font-bold text-white flex items-center gap-3 uppercase tracking-widest">
              <Gavel className="h-4 w-4 text-[#818258]" /> Próximas Audiências
            </CardTitle>
            <Link href="/agenda" className="text-[9px] font-bold text-[#818258] uppercase tracking-widest hover:text-white transition-colors">Ver Pauta Completa</Link>
          </CardHeader>
          <CardContent className="flex flex-col items-center justify-center h-full">
            {recentHearings && recentHearings.length > 0 ? (
              <div className="w-full space-y-4">
                {recentHearings.map((hearing, i) => (
                  <div key={i} className="flex items-center justify-between p-5 rounded-lg bg-white/5 border border-white/5 hover:border-primary/20 transition-colors group">
                    <div className="flex gap-5 items-center">
                      <div className="text-[#818258] font-bold text-xs bg-[#818258]/5 h-10 w-16 flex items-center justify-center rounded border border-[#818258]/10">
                        {hearing.startDateTime ? new Date(hearing.startDateTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : "--:--"}
                      </div>
                      <div>
                        <div className="font-bold text-white text-sm uppercase tracking-tight">{hearing.title}</div>
              ...
              <div className="text-center py-20 text-muted-foreground italic text-sm opacity-40">
        ...
        <Card className="glass border-white/5 shadow-2xl">
          <CardHeader className="flex flex-row items-center gap-3 border-b border-white/5 mb-6">
            <AlertCircle className="h-4 w-4 text-destructive/70" />
            <CardTitle className="text-xs font-bold text-white uppercase tracking-widest">Alertas Críticos</CardTitle>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="space-y-4">
              {deadlines && deadlines.length > 0 ? (
                ...
                <div className="p-6 rounded-lg bg-emerald-500/5 border-l-4 border-emerald-500/60 space-y-2">
                  <div className="font-bold text-emerald-500 uppercase tracking-widest text-[10px]">Tudo em Dia</div>
                  <p className="text-emerald-500/70 text-xs font-medium">Nenhum alerta crítico pendente no radar.</p>
                </div>
              )}
            </div>
            <Button variant="ghost" asChild className="w-full mt-2 text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground hover:text-[#818258] hover:bg-[#818258]/5">
              <Link href="/deadlines" className="flex items-center justify-center">Ver todos os alertas <ChevronRight className="h-3 w-3 ml-2" /></Link>
            </Button>
          </CardContent>
        </Card>
      </div>
