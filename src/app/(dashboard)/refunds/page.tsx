...
import { ArrowLeftRight, Plus, Receipt, Clock, CheckCircle2 } from "lucide-react"
import { Button } from "@/components/ui/button"

const refunds = [
  { id: "REF-01", description: "Custas Processuais - Vara 04", value: 350.20, date: "12/05/2024", status: "Aprovado" },
  { id: "REF-02", description: "Estacionamento - Fórum Barra Funda", value: 45.00, date: "14/05/2024", status: "Pendente" },
  { id: "REF-03", description: "Cópia de Processo Físico", value: 120.00, date: "10/05/2024", status: "Pago" },
]
          ...
          <Card className="glass border-primary/10">
            <CardHeader>
              <CardTitle className="text-lg font-headline text-white flex items-center gap-2">
                <Receipt className="h-5 w-5 text-primary" /> Novas Solicitações
              </CardTitle>
            </CardHeader>
              ...
              {refunds.map((ref, i) => (
                <div key={i} className="p-4 rounded-xl bg-secondary/30 border border-primary/5 hover-gold transition-all">
                  <div className="flex justify-between items-start mb-2">
                    <h5 className="font-bold text-white">{ref.description}</h5>
                  ...
                  <div className="flex justify-between items-center text-[10px] uppercase tracking-widest font-bold text-muted-foreground">
                    <span>{ref.date} • {ref.id}</span>
                    <Badge variant={ref.status === 'Pago' ? 'default' : 'outline'} className={ref.status === 'Pago' ? 'bg-emerald-500' : ''}>
                      {ref.status}
                    </Badge>
                  </div>
                </div>
            ...
            <CardContent className="space-y-8">
              <div className="flex items-center justify-between p-6 rounded-2xl bg-secondary/50 border border-primary/10">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-[10px] uppercase font-bold text-muted-foreground">Aguardando Aprovação</p>
                    <p className="text-2xl font-bold text-white">R$ 1.450,00</p>
                  </div>
                </div>
              ...
              <div className="flex items-center justify-between p-6 rounded-2xl bg-emerald-500/10 border border-emerald-500/20">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-[10px] uppercase font-bold text-muted-foreground">Pagos este Mês</p>
                    <p className="text-2xl font-bold text-white">R$ 3.820,00</p>
                  </div>
                </div>
                <ArrowLeftRight className="h-6 w-6 text-muted-foreground opacity-20" />
              </div>
