  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-500 font-sans">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-3">
        ...
        <Button onClick={() => setIsNewEntryOpen(true)} className="gold-gradient text-background font-bold gap-2 px-4 h-9 rounded-md uppercase text-[10px] tracking-wider">
              ...
              <div key={col.id} className="min-w-[240px] flex-1">
                <div className="flex items-center justify-between mb-3 px-1">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className={`font-bold text-[10px] tracking-wider uppercase ${col.color}`}>{col.title}</h3>
                  </div>
                    ...
                    <Card key={lead.id} className="glass hover-gold transition-all cursor-pointer group border-white/5 shadow-sm" onClick={() => handleOpenLead(lead)}>
                      <CardContent className="p-3 space-y-2">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          {lead.meetingType === 'online' && <Video className="h-3 w-3 text-primary" />}
                        </div>
                        {lead.scheduledDate && (
                          <div className="flex items-center gap-1.5 px-1.5 py-1 rounded bg-amber-500/5 border border-amber-500/10">
        ...
        <SheetContent className="w-full lg:w-[calc(100vw-16rem)] lg:max-w-none overflow-hidden glass border-l border-white/10 p-0 flex flex-col bg-[#05070a]">
          {selectedLead && (
            <div className="flex flex-col h-full">
              <div className="p-4 md:p-6 border-b border-white/5 bg-[#0a0f1e]/80 backdrop-blur-xl space-y-4 flex-none shadow-lg">
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                  <div className="space-y-2 flex-1">
                    <div className="flex flex-wrap items-center gap-2">
                      <Badge variant="outline" className="text-[9px] border-primary/20 text-primary uppercase font-bold px-2 py-0.5 tracking-wider bg-primary/5">
                        {(selectedLead.status || "novo").toUpperCase()}
                      </Badge>
                      <div className={cn("flex items-center gap-1 px-1.5 py-0.5 rounded-full border", isAlreadyClient ? "bg-emerald-500/10 border-emerald-500/20" : "bg-amber-500/10 border-amber-500/20")}>
                        <Fingerprint className={cn("h-2.5 w-2.5", isAlreadyClient ? "text-emerald-500" : "text-amber-500")} />
                        <span className={cn("text-[8px] font-bold uppercase tracking-wider", isAlreadyClient ? "text-emerald-500" : "text-amber-500")}>
                          {isAlreadyClient ? "VINCULADO" : "PENDENTE"}
                        </span>
                      </div>
                    </div>
                    <h2 className="text-lg md:text-xl font-bold text-white uppercase tracking-tight">{selectedLead.name}</h2>
                    <div className="flex flex-wrap items-center gap-1.5">
                      <Button onClick={() => setIsEditModeOpen(true)} variant="outline" className="h-7 border-white/10 bg-white/5 text-white text-[9px] font-bold uppercase px-3 rounded">
                        <UserCog className="h-3 w-3 mr-1.5" /> EDITAR
                      </Button>
                      <Button onClick={handleSyncDrive} disabled={isSyncingDrive} variant="outline" className="h-7 border-amber-500/20 bg-amber-500/5 text-amber-500 text-[9px] font-bold uppercase px-3 rounded">
                        {isSyncingDrive ? <Loader2 className="h-3 w-3 animate-spin" /> : <CloudLightning className="h-3 w-3 mr-1.5" />} DRIVE
                      </Button>
                      <Button onClick={handleArchiveLead} variant="outline" className="h-7 border-blue-500/20 bg-blue-500/5 text-blue-400 text-[9px] font-bold uppercase px-3 rounded">
                        <Archive className="h-3 w-3 mr-1.5" /> ARQUIVAR
                      </Button>
                      <Button onClick={handleDeleteLead} variant="outline" className="h-7 border-rose-500/20 bg-rose-500/5 text-rose-500 text-[9px] font-bold uppercase px-3 rounded">
                        <Trash2 className="h-3 w-3 mr-1.5" /> EXCLUIR
                      </Button>
                    </div>
                  </div>
                  <div className="bg-black/40 border border-white/5 p-1 rounded-lg flex flex-col gap-0.5 w-full lg:w-40">
                    {columns.map(c => (
                      <button key={c.id} onClick={() => handleUpdateStatus(c.id)} className={cn("h-7 px-2 text-[9px] font-bold uppercase tracking-wider rounded transition-all flex items-center justify-between", selectedLead.status === c.id ? "bg-white/10 text-white" : "text-muted-foreground hover:text-white")}>
                        {c.title} {selectedLead.status === c.id && <CheckCircle2 className="h-2.5 w-2.5 text-primary" />}
                      </button>
                    ))}
                  </div>
                </div>
              ...
              <ScrollArea className="flex-1 bg-[#05070a]">
                <div className="p-4 md:p-6 pb-20 space-y-6 max-w-5xl mx-auto">
                  {isProfileIncomplete && (
                    <div className="p-3 rounded-lg bg-amber-500/5 border border-amber-500/20 flex items-center justify-between gap-3">
                      <div className="flex items-center gap-2">
                        <ShieldAlert className="h-4 w-4 text-amber-500" />
                        <p className="text-[9px] font-bold text-white uppercase tracking-wider">Pendências: <span className="text-amber-400">{missingFields.join(", ")}</span></p>
                      </div>
                      <Button onClick={() => setIsEditModeOpen(true)} className="h-6 px-3 gold-gradient text-background font-bold uppercase text-[8px] rounded">SANEAR</Button>
                    </div>
                  )}

                  <Tabs value={activeDossierTab} onValueChange={setActiveDossierTab} className="space-y-4">
                    <TabsList className="bg-transparent border-b border-white/5 h-8 w-full justify-start rounded-none p-0 gap-4">
                    ...
                    <TabsContent value="overview" className="space-y-4 animate-in fade-in duration-300 outline-none">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <Card className="glass border-white/5 p-4 rounded-xl">
                          <h4 className="text-[9px] font-bold text-amber-500 uppercase tracking-wider flex items-center gap-2 mb-3"><Clock className="h-3 w-3" /> AGENDAMENTO</h4>
                          <div className="space-y-2">
                            <p className="text-[11px] font-bold text-white uppercase">{selectedLead.scheduledDate ? `${new Date(selectedLead.scheduledDate).toLocaleDateString()} ÀS ${selectedLead.scheduledTime}` : "SEM AGENDAMENTO"}</p>
                            <p className="text-[9px] text-muted-foreground uppercase font-bold">{selectedLead.meetingType === 'online' ? '🖥️ VIRTUAL' : '🏢 PRESENCIAL'}</p>
                          </div>
                        </Card>
                        <Card className="glass border-primary/10 p-4 rounded-xl">
                          <h4 className="text-[9px] font-bold text-primary uppercase tracking-wider flex items-center gap-2 mb-3"><Brain className="h-3 w-3" /> SÍNTESE IA</h4>
                          <p className="text-[10px] text-white/70 leading-relaxed italic line-clamp-3">{selectedLead.aiSummary || "Aguardando consolidação..."}</p>
                        </Card>
                      </div>
                    </TabsContent>

                    <TabsContent value="entrevistas" className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      {templates?.map(t => (
                        <Button key={t.id} onClick={() => handleStartInterview(t)} variant="outline" className="glass border-primary/10 text-primary font-bold uppercase text-[9px] h-10 gap-2 rounded-lg justify-start px-3">
                    ...
                    <TabsContent value="revisão" className="space-y-6">
                      <div className="flex items-center justify-between pb-3 border-b border-white/5">
                        <h3 className="text-sm font-bold text-white uppercase tracking-tight flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-primary" /> Check-in de Protocolo</h3>
                        <Button onClick={handleGenerateStrategicSummary} disabled={isGeneratingSummary} className="h-8 px-4 glass border-primary/20 text-primary font-bold uppercase text-[9px] gap-2 rounded">
                          {isGeneratingSummary ? <Loader2 className="h-3 w-3 animate-spin" /> : <Brain className="h-3 w-3" />} CONSOLIDAR
                        </Button>
                      </div>
                      ...
                      {strategicSummary && (
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
                          {[
                            ...
                            <div key={s.t} className="p-3 rounded-lg bg-white/[0.02] border border-white/5">
                              <h5 className={cn("text-[8px] font-bold uppercase tracking-wider mb-1.5", s.c)}>{s.t}</h5>
                              <p className="text-[10px] text-white/60 leading-relaxed">{s.v}</p>
                            </div>
                          ))}
                        </div>
                      )}
                      ...
                      <div className="p-4 rounded-xl bg-purple-500/5 border border-purple-500/10 space-y-3">
                        <Label className="text-[9px] font-bold text-muted-foreground uppercase">NÚMERO DO PROCESSO (CNJ) *</Label>
                        <div className="flex gap-2">
                          <Input placeholder="0000000-00.0000.0.00.0000" className="glass border-white/10 h-10 text-white font-mono text-sm font-bold" value={selectedLead.processNumber || ""} onChange={(e) => setSelectedLead({...selectedLead, processNumber: e.target.value})} />
                          <Button onClick={() => setIsConversionOpen(true)} className="gold-gradient text-background font-bold h-10 px-6 rounded-lg uppercase text-[9px] tracking-wider">CONVERTER</Button>
                        </div>
                      </div>
                    </TabsContent>
                  </Tabs>
      ...
      <Sheet open={isNewEntryOpen} onOpenChange={setIsNewEntryOpen}>
        <SheetContent className="w-full lg:w-[calc(100vw-16rem)] lg:max-w-none overflow-hidden glass border-l border-white/10 p-0 flex flex-col bg-[#05070a]">
          <div className="p-4 border-b border-white/5"><h2 className="text-sm font-bold text-white uppercase tracking-tight">Novo Atendimento RGMJ</h2></div>
          <LeadForm 
            existingLeads={leads} 
            onSubmit={(data) => {
      ...
      <Sheet open={isEditModeOpen} onOpenChange={setIsEditModeOpen}>
        <SheetContent className="w-full lg:w-[calc(100vw-16rem)] lg:max-w-none overflow-hidden glass border-l border-white/10 p-0 flex flex-col bg-[#05070a]">
          <div className="p-4 border-b border-white/5"><h2 className="text-sm font-bold text-white uppercase tracking-tight">Saneamento de Dossiê</h2></div>
          {selectedLead && (
            <LeadForm 
              existingLeads={[]}
