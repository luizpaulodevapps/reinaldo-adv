  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 font-sans">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-xs uppercase tracking-widest font-black text-muted-foreground/40 mb-2">
        ...
        <Button onClick={() => setIsNewEntryOpen(true)} className="gold-gradient text-background font-black gap-2 px-6 h-12 rounded-lg text-xs tracking-widest shadow-xl">
      ...
      {isLoading ? (
        <div className="py-24 flex flex-col items-center justify-center space-y-4">
          ...
          <span className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Sincronizando Banco de Dados...</span>
        </div>
      ) : (
              ...
              <div key={col.id} className="min-w-[320px] flex-1 flex flex-col">
                <div className="flex items-center justify-between mb-4 px-2">
                  <div className="flex items-center gap-3">
                    ...
                    <h3 className={`font-black text-xs tracking-widest uppercase ${col.color}`}>{col.title}</h3>
                  </div>
                  <Badge variant="secondary" className="bg-white/5 text-xs border-white/5 font-black h-6 px-2">{leadsInCol.length}</Badge>
                </div>
                <div className="space-y-4 flex-1 bg-white/[0.01] rounded-2xl p-2 border border-white/5">
                  {leadsInCol.map((lead) => (
                    <Card key={lead.id} className="glass hover-gold transition-all cursor-pointer group border-white/5 shadow-xl" onClick={() => handleOpenLead(lead)}>
                      <CardContent className="p-4 space-y-4">
                        <div className="flex items-start justify-between gap-3">
                          <div className="font-bold text-sm text-white group-hover:text-primary transition-colors uppercase tracking-tight flex-1 leading-snug">{lead.name}</div>
                          {lead.meetingType === 'online' && <Video className="h-4 w-4 text-primary shrink-0" />}
                        </div>
                        
                        <div className="flex flex-col gap-2">
                          {lead.scheduledDate && (
                            <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-amber-500/5 border border-amber-500/10 w-fit">
                          ...
                          <div className="flex items-center gap-2 text-[10px] text-muted-foreground/60 uppercase font-black tracking-widest">
                            <Building className="h-3.5 w-3.5" /> {lead.defendantName || "Réu não informado"}
                          </div>
                        </div>
                        ...
                        <div className="flex items-center justify-between pt-4 border-t border-white/5">
                          ...
                          <div className="flex items-center gap-1.5 text-primary font-black text-[10px] uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-all">
                  ...
                  {leadsInCol.length === 0 && (
                    <div className="h-32 flex items-center justify-center border-2 border-dashed border-white/5 rounded-xl opacity-20">
          ...
          {selectedLead && (
            <div className="flex flex-col h-full">
              <SheetHeader className="p-6 border-b border-white/5 bg-[#0a0f1e] z-10 flex-none shadow-xl">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-6">
                    ...
                    <div className="space-y-1">
                      ...
                      <SheetDescription asChild>
                        <span className="text-xs text-muted-foreground uppercase font-black tracking-[0.3em] flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-4">
                    <Button onClick={() => handleSyncDrive()} disabled={isSyncingDrive} variant="outline" className="h-11 border-white/10 bg-white/5 text-xs font-black uppercase px-6 rounded-lg gap-3 transition-all hover:bg-primary/5 shadow-xl">
                    ...
                    <Button onClick={() => setIsEditModeOpen(true)} variant="outline" className="h-11 border-white/10 bg-white/5 text-xs font-black uppercase px-6 rounded-lg gap-3 transition-all hover:bg-primary/5 shadow-xl">
              ...
              <ScrollArea className="flex-1 bg-[#05070a]">
                <div className="p-8 space-y-8 w-full max-w-[1600px] mx-auto pb-32">
                  <Tabs value={activeDossierTab} onValueChange={setActiveDossierTab} className="space-y-8">
                    <TabsList className="bg-transparent border-b border-white/5 h-12 w-full justify-start rounded-none p-0 gap-12 flex-none overflow-x-auto scrollbar-hide">
                      {[
                        ...
                        <TabsTrigger key={tab.id} value={tab.id} className="data-[state=active]:text-primary text-muted-foreground font-black text-xs uppercase h-full rounded-none px-0 border-b-4 border-transparent data-[state=active]:border-primary transition-all tracking-widest">{tab.label}</TabsTrigger>
                      ))}
                    </TabsList>

                    <TabsContent value="overview" className="space-y-8 animate-in fade-in duration-300 outline-none w-full">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <Card className="glass border-white/5 p-8 rounded-2xl shadow-2xl bg-white/[0.01]">
                          <h4 className="text-xs font-black text-amber-500 uppercase tracking-widest flex items-center gap-3 mb-6"><Clock className="h-5 w-5" /> Agenda do Atendimento</h4>
                          <div className="space-y-4">
                            ...
                            <Badge variant="outline" className="text-xs font-black text-muted-foreground border-white/10 px-4 py-2 uppercase tracking-widest">
                              {selectedLead.meetingType === 'online' ? '🖥️ REUNIÃO VIRTUAL' : '🏢 VISITA PRESENCIAL'}
                            </Badge>
                          </div>
                        </Card>
                        <Card className="glass border-primary/15 p-8 rounded-2xl shadow-2xl bg-primary/5">
                          <h4 className="text-xs font-black text-primary uppercase tracking-widest flex items-center gap-3 mb-6"><Brain className="h-5 w-5" /> Síntese Preliminar (IA)</h4>
                          <p className="text-sm text-white/80 leading-relaxed italic text-justify">
                    ...
                    <TabsContent value="entrevistas" className="w-full">
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                        {templates?.map(t => (
                          <Button key={t.id} onClick={() => handleStartInterview(t)} variant="outline" className="glass border-primary/10 text-primary font-black uppercase text-xs h-16 gap-4 rounded-xl justify-start px-6 hover:bg-primary/5 transition-all shadow-lg group">
                    ...
                    <TabsContent value="revisao" className="space-y-8 animate-in fade-in duration-500 w-full">
                      <div className="flex items-center justify-between pb-6 border-b border-white/5">
                        <div className="flex items-center gap-4">
                          <div className="w-12 h-12 rounded-full bg-emerald-500/10 flex items-center justify-center border border-emerald-500/20">
                            <ShieldCheck className="h-6 w-6 text-emerald-500" />
                          </div>
                          <h3 className="text-xl font-bold text-white uppercase tracking-widest">Check-in de Protocolo</h3>
                        </div>
                        <Button onClick={handleGenerateStrategicSummary} disabled={isGeneratingSummary} className="h-12 px-8 glass border-primary/20 text-primary font-black uppercase text-xs gap-3 rounded-lg shadow-2xl hover:bg-primary/5">
                          {isGeneratingSummary ? <Loader2 className="h-5 w-5 animate-spin" /> : <Brain className="h-5 w-5" />} CONSOLIDAR ESTRATÉGIA IA
                        </Button>
                      </div>

                      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        <Card className="glass border-white/5 p-8 rounded-2xl space-y-6 shadow-2xl bg-white/[0.01]">
                          <div className="flex items-center gap-4 mb-2">
                            <User className="h-5 w-5 text-primary" />
                            <span className="text-xs font-black text-white uppercase tracking-widest">Autor</span>
                          </div>
                          <div className="space-y-4">
                            {[
                              { label: "Nome", value: selectedLead.name },
                              { label: "CPF", value: selectedLead.cpf || selectedLead.documentNumber || "NÃO INFORMADO" },
                        ...
                        <Card className="glass border-white/5 p-8 rounded-2xl space-y-6 shadow-2xl bg-white/[0.01]">
                          <div className="flex items-center gap-4 mb-2">
                            ...
                            <span className="text-xs font-black text-white uppercase tracking-widest">Réu</span>
                          </div>
                          <div className="space-y-4">
                            {[
                              { label: "Empresa", value: selectedLead.defendantName || "NÃO INFORMADO" },
                        ...
                        <Card className="glass border-white/5 p-8 rounded-2xl space-y-6 shadow-2xl bg-white/[0.01]">
                          <div className="flex items-center gap-4 mb-2">
                            ...
                            <span className="text-xs font-black text-white uppercase tracking-widest">Logística</span>
                          </div>
                          <div className="space-y-4">
                            {[
                      ...
                      <Card className="glass border-white/5 overflow-hidden shadow-2xl rounded-2xl">
                        <div className="p-6 bg-white/[0.02] border-b border-white/5 flex items-center justify-between">
                          <div className="flex items-center gap-4">
                            ...
                            <span className="text-base font-bold text-white uppercase tracking-widest">Histórico de Fatos</span>
                          </div>
                          <Badge variant="outline" className="text-[10px] font-black border-primary/20 text-primary px-4 py-1 uppercase">{leadInterviews?.length || 0} Atendimentos</Badge>
                        </div>
                        <div className="p-8">
                          {leadInterviews && leadInterviews.length > 0 ? (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                              {leadInterviews.slice(0, 4).map((int, idx) => (
                                <div key={idx} className="p-6 rounded-2xl bg-black/30 border border-white/5 space-y-4 shadow-xl">
                                  <div className="flex items-center justify-between border-b border-white/5 pb-3">
                                    <span className="text-xs font-bold text-primary uppercase tracking-widest">{int.interviewType}</span>
                                    <span className="text-[10px] text-muted-foreground font-mono">{int.createdAt?.toDate ? new Date(int.createdAt.toDate()).toLocaleDateString('pt-BR') : '---'}</span>
                                  </div>
                                  <div className="space-y-4">
                                    {Object.entries(int.responses || {}).slice(0, 2).map(([q, a]: any) => (
                                      <div key={q}>
                                        <p className="text-[10px] font-black text-muted-foreground uppercase opacity-50 mb-1">{q}</p>
                                        <p className="text-xs text-white/90 font-medium uppercase leading-relaxed">{String(a)}</p>
                                      </div>
                                    ))}
                                  </div>
                                </div>
                            ...
                            <div className="py-16 flex flex-col items-center justify-center opacity-20 space-y-4">
                              <MessageCircle className="h-12 w-12" />
                              <p className="text-xs font-black uppercase tracking-widest">Sem atendimentos registrados.</p>
                            </div>
                          )}
                        </div>
                      </Card>
                      ...
                      {strategicSummary && (
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in slide-in-from-bottom-4 duration-700">
                          {[
                            ...
                            { t: "Riscos", v: strategicSummary.risksAndChallenges, c: "text-rose-500", icon: ShieldAlert },
                            { t: "Estratégia", v: strategicSummary.strategicAnalysis, icon: Zap, c: "text-emerald-500" }
                          ].map(s => (
                            <div key={s.t} className="p-8 rounded-2xl bg-white/[0.02] border border-white/5 space-y-4 shadow-xl hover:border-primary/20 transition-all">
                              <div className="flex items-center gap-3">
                                <s.icon className={cn("h-6 w-6", s.c)} />
                                <h5 className={cn("text-xs font-black uppercase tracking-widest", s.c)}>{s.t}</h5>
                              </div>
                              <p className="text-sm text-white/80 leading-relaxed text-justify">{s.v}</p>
                            </div>
                          ))}
                        </div>
                      )}

                      <Card className="glass border-amber-500/20 bg-amber-500/5 p-6 rounded-2xl space-y-6 shadow-2xl">
                        <div className="flex items-center justify-between border-b border-amber-500/10 pb-4">
                          <div className="flex items-center gap-4">
                            ...
                            <div className="space-y-1">
                              <h3 className="text-xl font-bold text-white uppercase tracking-widest">Agendar Audiência</h3>
                              <p className="text-[10px] text-amber-500/60 uppercase font-bold tracking-widest">Opcional: Sincronismo com agenda RGMJ</p>
                            </div>
                          </div>
                          <div className="flex items-center gap-3 bg-black/40 p-3 rounded-xl border border-white/5">
                            <span className="text-[10px] font-black text-white uppercase tracking-widest">Habilitar?</span>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 animate-in slide-in-from-top-2 duration-500">
                            <div className="space-y-2">
                              ...
                              <Select value={hearingData.type} onValueChange={(v) => setHearingData({...hearingData, type: v})}>
                                <SelectTrigger className="bg-black/40 border-white/10 h-11 text-white font-bold text-xs"><SelectValue /></SelectTrigger>
                            ...
                            <div className="space-y-2">
                              ...
                              <Input type="date" className="bg-black/40 border-white/10 h-11 text-white font-bold text-xs" value={hearingData.date} onChange={(e) => setHearingData({...hearingData, date: e.target.value})} />
                            </div>
                            <div className="space-y-2">
                              <Label className="text-[10px] font-black text-muted-foreground uppercase">Horário</Label>
                              <Input type="time" className="bg-black/40 border-white/10 h-11 text-white font-bold text-xs" value={hearingData.time} onChange={(e) => setHearingData({...hearingData, time: e.target.value})} />
                            </div>
                            <div className="space-y-2">
                              <Label className="text-[10px] font-black text-muted-foreground uppercase">Local</Label>
                              <Input 
                                placeholder={hearingData.type === 'Virtual' ? "Sala Virtual RGMJ" : "Fórum Trabalhista"}
                                className="bg-black/40 border-white/10 h-11 text-white font-bold text-xs" 
                            ...
                            {hearingData.type === 'Virtual' && (
                              <div className="col-span-full grid grid-cols-1 md:grid-cols-2 gap-6 pt-2 border-t border-white/5">
                                <div className="space-y-2">
                                  <Label className="text-[10px] font-black text-emerald-500 uppercase flex items-center gap-2">
                                    <Video className="h-3.5 w-3.5" /> Link da Sala
                                  </Label>
                                  <Input className="bg-black/40 border-emerald-500/20 h-11 text-white font-bold text-xs" placeholder="https://zoom.us/j/..." value={hearingData.meetingLink} onChange={(e) => setHearingData({...hearingData, meetingLink: e.target.value})} />
                                </div>
                                <div className="space-y-2">
                                  <Label className="text-[10px] font-black text-emerald-500 uppercase flex items-center gap-2">
                                    <Lock className="h-3.5 w-3.5" /> Senha
                                  </Label>
                                  <Input className="bg-black/40 border-emerald-500/20 h-11 text-white font-bold text-xs" placeholder="Senha de acesso" value={hearingData.accessCode} onChange={(e) => setHearingData({...hearingData, accessCode: e.target.value})} />
                                </div>
                              </div>
                            )}
                          </div>
                      ...
                      <div className="p-8 rounded-2xl bg-purple-500/5 border border-purple-500/15 flex flex-col md:flex-row items-end gap-8 w-full shadow-2xl">
                        <div className="flex-1 space-y-3 w-full">
                          <Label className="text-[10px] font-black text-purple-400 uppercase tracking-widest flex items-center gap-3">
                            <FileCheck className="h-5 w-5" /> CNJ DO PROCESSO *
                          </Label>
                          <Input 
                            placeholder="0000000-00.0000.0.00.0000" 
                            className="bg-black/60 border-purple-500/20 h-16 text-white font-mono text-xl font-black w-full tracking-widest focus:ring-purple-500/50 rounded-xl px-6" 
                        ...
                        <Button 
                          ...
                          className="gold-gradient text-background font-black h-16 px-12 rounded-xl uppercase text-xs tracking-widest shadow-2xl hover:scale-[1.02] active:scale-95 transition-all shrink-0 w-full md:w-auto"
                        >
              ...
              <div className="flex-none p-4 border-t border-white/5 bg-[#0a0f1e] flex items-center justify-between z-20 shadow-2xl">
                <Button 
                  ...
                  disabled={!canGoBack}
                  className="text-muted-foreground uppercase font-black text-[10px] tracking-widest gap-2 px-6 h-11 hover:text-white disabled:opacity-20"
                >
                ...
                {canGoNext ? (
                  <Button 
                    ...
                    className="gold-gradient text-background font-black h-11 px-10 rounded-lg uppercase text-[10px] tracking-widest gap-2 shadow-2xl hover:scale-[1.02] transition-all"
                  >
                    PRÓXIMO PASSO <ArrowRight className="h-4 w-4" />
                  </Button>
                ) : (
                  <div className="px-6 h-11 flex items-center">
                    <span className="text-[9px] font-black text-primary/40 uppercase tracking-widest">Protocolo Final</span>
                  </div>
                )}
              </div>
            </div>
      ...
      <Dialog open={isInterviewDialogOpen} onOpenChange={setIsInterviewDialogOpen}>
        <DialogContent className="glass border-white/10 bg-[#05070a] sm:max-w-[1000px] w-[95vw] p-0 overflow-hidden shadow-2xl">
          <DialogHeader className="p-6 bg-[#0a0f1e] border-b border-white/5">
            <DialogTitle className="text-white text-xl font-bold uppercase tracking-tight">Atendimento Técnico</DialogTitle>
            <DialogDescription className="text-[10px] text-muted-foreground uppercase font-black tracking-widest mt-2">Captura estruturada de fatos RGMJ</DialogDescription>
          </DialogHeader>
          {executingTemplate && (
      ...
      <Dialog open={isConversionOpen} onOpenChange={setIsConversionOpen}>
        <DialogContent className="glass border-white/10 bg-[#05070a] sm:max-w-[1000px] w-[95vw] p-0 overflow-hidden shadow-2xl">
          <DialogHeader className="p-6 bg-[#0a0f1e] border-b border-white/5">
            <DialogTitle className="text-white text-xl font-bold uppercase tracking-tight">Migração para Ativos</DialogTitle>
            <DialogDescription className="text-[10px] text-muted-foreground uppercase font-black tracking-widest mt-2">O dossiê do lead será encerrado e um novo processo será iniciado.</DialogDescription>
          </DialogHeader>
          {selectedLead && (
            <ProcessForm 
              initialData={{
      ...
      <Sheet open={isNewEntryOpen} onOpenChange={setIsNewEntryOpen}>
        <SheetContent className="w-full lg:w-[calc(100vw-16rem)] lg:max-w-none overflow-hidden glass border-l border-white/10 p-0 flex flex-col bg-[#05070a] shadow-2xl">
          <SheetHeader className="p-6 border-b border-white/5 bg-[#0a0f1e] shadow-xl">
            <SheetTitle className="text-xl font-bold text-white uppercase tracking-tight">Novo Atendimento RGMJ</SheetTitle>
            <SheetDescription className="text-[10px] text-muted-foreground uppercase font-black tracking-widest mt-2">Cadastro de nova oportunidade no funil comercial</SheetDescription>
          </SheetHeader>
          <LeadForm 
            existingLeads={leads} 
            onSubmit={(data) => {
      ...
      <Sheet open={isEditModeOpen} onOpenChange={setIsEditModeOpen}>
        <SheetContent className="w-full lg:w-[calc(100vw-16rem)] lg:max-w-none overflow-hidden glass border-l border-white/10 p-0 flex flex-col bg-[#05070a] shadow-2xl">
          <SheetHeader className="p-6 border-b border-white/5 bg-[#0a0f1e] shadow-xl">
            <SheetTitle className="text-xl font-bold text-white uppercase tracking-tight">Saneamento de Dossiê</SheetTitle>
            <SheetDescription className="text-[10px] text-muted-foreground uppercase font-black tracking-widest mt-2">Retificação técnica de dados cadastrais</SheetDescription>
          </SheetHeader>
          {selectedLead && (
            <LeadForm 
              existingLeads={[]}
