  ...
  return (
    <div className="space-y-8">
      ...
      <Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen}>
        <SheetContent className="w-full sm:max-w-2xl glass border-l border-primary/20 p-0 flex flex-col bg-[#0a0f1e]">
          {selectedLead && (
            <>
                        ...
                        <div className="space-y-6">
                          <div className="p-6 rounded-2xl bg-primary/5 border border-primary/20">
                            <h3 className="text-[#f5d030] font-black text-xs uppercase tracking-[0.2em] flex items-center gap-2 mb-2">
                              <Brain className="h-4 w-4" /> Script Sugerido: {activeInterview.title}
                            </h3>
                            <p className="text-[11px] text-muted-foreground leading-relaxed italic">
                              "{activeInterview.description || "Utilize este roteiro para garantir o máximo de performance no atendimento."}"
                            </p>
                          </div>

                          <div className="space-y-3">
                            {activeInterview.items?.map((item: any, i: number) => (
                              <div key={i} className="flex items-center gap-4 p-4 rounded-xl bg-white/[0.02] border border-white/5 hover:border-primary/20 transition-all">
                                <Checkbox id={`step-${i}`} className="border-primary/50 data-[state=checked]:bg-primary" />
                                <label htmlFor={`step-${i}`} className="text-sm font-bold text-white/80 cursor-pointer uppercase tracking-tight">
                                  {item.text}
                                </label>
                              </div>
                            ))}
