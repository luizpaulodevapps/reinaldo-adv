...
export default function ChecklistExecutionPage() {
    ...
    addDocumentNonBlocking(collection(db, "checklist_executions"), newExecution)
      .then(() => {
        ...
        toast({
          ...
          description: `Você iniciou a execução de: ${template.title}`,
        })
      })
  }
  ...
  const handleMarkComplete = (executionId: string) => {
    ...
    toast({ title: "Auditoria Finalizada", description: "O protocolo foi arquivado como concluído." })
  }
    ...
    <div className="space-y-10 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        <div className="space-y-1">
          <h1 className="text-5xl font-headline font-bold text-white tracking-tighter">Execução de Protocolos</h1>
          <p className="text-muted-foreground uppercase tracking-[0.3em] text-[10px] font-black opacity-60">Aplicação técnica das matrizes estratégicas RGMJ.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {isLoading ? (
          <div className="col-span-full py-32 flex flex-col items-center justify-center space-y-4">
            ...
            <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Sincronizando Auditorias...</span>
          </div>
              ...
              <CardContent className="p-8 space-y-6">
                ...
                <div className="flex items-center justify-between pt-2">
                  ...
                  {exec.status !== 'Finalizado' && (
                    ...
                    >
                      <ShieldCheck className="h-4 w-4" /> Finalizar Ato
                    </button>
                  )}
                </div>
              </CardContent>
          ...
          <div className="col-span-full py-32 flex flex-col items-center justify-center space-y-6 glass rounded-3xl border-dashed border-2 border-white/5 opacity-30">
            ...
            <div className="text-center space-y-2">
              <p className="text-sm font-bold text-white uppercase tracking-widest">Nenhuma Auditoria Ativa</p>
              <p className="text-xs text-muted-foreground max-w-xs mx-auto">Sua banca está em conformidade. Nenhum protocolo operacional em aberto no momento.</p>
            </div>
          ...
          <div className="p-8 bg-[#0a0f1e] border-b border-white/5">
            <DialogHeader>
              <DialogTitle className="text-white font-headline text-3xl uppercase tracking-tighter">
                Selecionar Matriz de Execução
              </DialogTitle>
              <p className="text-muted-foreground text-[10px] uppercase font-bold tracking-[0.2em] mt-1">
                Escolha o protocolo padrão para iniciar a auditoria técnica.
              </p>
            </DialogHeader>
          </div>
